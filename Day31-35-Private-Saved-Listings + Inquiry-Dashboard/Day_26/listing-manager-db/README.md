...
## Day 31 auth checkpoint

The app now has a `users` table and two auth routes.
...
If you already have listings in your database, run only the Day 31 auth schema from the `listing-manager-db` folder:

```powershell
psql -d listing_manager -f auth-schema.sql
```

Test signup with this JSON body:
...
Plain password from request
  -> backend creates password_hash
  -> users table stores password_hash
  -> login compares typed password with saved hash
```
